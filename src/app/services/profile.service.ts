...
export class ProfileService {
  ...
  loadProfile(): Observable<void> {
    ...
    return this.http.post<any>(`${this.apiUrl}/api/profile`, { university_id: uid }).pipe(
      tap((res) => {
        const p = res.profile ?? res.student ?? res;
        ...
        const student: Student = {
          ...
          yearOfStudy: p.year_of_study ?? p.yearOfStudy ?? current?.yearOfStudy ?? '',
          department: p.department ?? current?.department ?? '',
          studentId: p.student_id ?? uid,
          gpa: p.gpa ?? current?.gpa ?? 0,
          passedHours: p.passed_hours ?? current?.passedHours ?? 0,
          availableHours: p.available_hours ?? current?.availableHours ?? 18,
          role: 'student',
        };
